...
public abstract class Packet {
    ...
    final static int SERVER_MESSAGE_INFO = 0x50005001;
    final static int SERVER_MESSAGE_WARNING = 0x50005002;
    final static int SERVER_MESSAGE_ERROR = 0x50005003;

    private static String defaultPolicy = "<cross-domain-policy><allow-access-from domain=\"*\" to-ports=\"*\"/></cross-domain-policy>";
        
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(Packet.class);
    ...
    public static Packet getPacketClass(int i) {
        try {
            ...
            log.error("Skipping packet with id " + i);
            return null;
        }
    }
    ...
    private void encryptPacket (IoBuffer buffer) {
        PublicKey key = RSAEncryptionManager.loadPublicKey();
        ...
        try {        
            ...
            byte[][] splittedData = splitArrayToEncode(buffer.array(), RSAEncryptionManager.KEY_SIZE_DEFAULT/8 - 11);
            //byte[] data = cipher.doFinal(buffer.array());  
            buffer.clear();
            
            for(int i = 0; i < splittedData.length; i++) {
                buffer.put(cipher.doFinal(splittedData[i]));
            }
                      
            //buffer.put(data); 
            
            log.info("Packet was successfully encrypt");
        } catch (Exception e) {
            log.error(e.getMessage());
        }
    }
        
    private byte[][] splitArrayToEncode(byte[] array, int maxSize) {
